...
# --- 1. THE PIPELINE LOGIC (This is stable and correct) ---
def run_pipeline(file_path: str, target_variable: str):
    ...
    try:
        ...
        all_results['ingestion'] = {'original_shape': ingestion.df.shape, 'head': ingestion.df.head().to_dict('records')}
        ...
        if problem_type == 'unsupervised':
            ...
            train_df, test_df = processed_df, processed_df
            all_results['feature_engineering'] = {'processed_shape': processed_df.shape, 'head': processed_df.head().to_dict('records')}
            training_results = train_and_evaluate_models(train_df, test_df, None, 'clustering', models_to_train)
        else:
            ...
            all_results['feature_engineering'] = {'processed_train_shape': train_df.shape, 'processed_test_shape': test_df.shape, 'head': train_df.head().to_dict('records')}
            
        training_results = train_and_evaluate_models(train_df, test_df, target_variable, problem_type, models_to_train)
        all_results['training_evaluation'] = training_results
        return all_results
    except Exception as e:
        return {"error": str(e)}

# --- 2. FINAL & IMPROVED HTML REPORT GENERATION ---
def generate_html_report(results: dict) -> str:
    """
    Generates a professional, styled, and concise HTML report from the pipeline results.
    This version correctly formats ALL EDA insights into tables and includes ALL steps.
    """
    problem_type = results.get('problem_type', 'N/A').capitalize()
    
    style = """
    <style>
        body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; margin: 20px; background-color: #f9fafb; color: #1f2937; }
        .container { max-width: 1000px; margin: auto; background-color: white; padding: 30px; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.08); }
        h1, h2, h3, h4 { color: #111827; border-bottom: 2px solid #e5e7eb; padding-bottom: 8px; }
        h1 { text-align: center; border: none; font-size: 2.5em; margin-bottom: 5px; }
        .section { margin-bottom: 30px; }
        table { width: 100%; border-collapse: collapse; margin-top: 15px; }
        th, td { text-align: left; padding: 10px 12px; border: 1px solid #d1d5db; }
        th { background-color: #3b82f6; color: white; }
        tr:nth-child(even) { background-color: #f9fafb; }
        img { max-width: 90%; height: auto; border-radius: 8px; margin: 20px auto; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        .dataframe { overflow-x: auto; }
        .subtitle { text-align: center; color: #6b7280; margin-top: 0; margin-bottom: 30px; font-size: 1.1em; }
    </style>
    """
    
    html = f"<h1>AutoML Pipeline Report</h1><p class='subtitle'><strong>Detected Problem Type:</strong> {problem_type}</p>"

    # --- Step 1 & 2: Ingestion & Processing ---
    html += "<div class='section'><h2>Step 1 & 2: Data Ingestion & Processing</h2>"
    ingestion_res = results.get('ingestion', {})
    processing_res = results.get('processing', {})
    html += f"<p><b>Original Shape:</b> {ingestion_res.get('original_shape')}</p>"
    html += f"<p><b>Shape After Cleaning:</b> {processing_res.get('cleaned_shape')}</p>"
    html += "<h4>Data Preview (Cleaned)</h4><div class='dataframe'>"
    html += pd.DataFrame(processing_res.get('head', {})).to_html(classes='styled-table')
    html += "</div></div>"

    # --- Step 3: EDA (with all insights as tables) ---
    html += "<div class='section'><h2>Step 3: Exploratory Data Analysis</h2>"
    insights = results.get('eda', {}).get('insights', {})
    
    # Create tables for each part of the insights
    if 'statistical_summary' in insights:
        html += "<h4>Statistical Summary</h4><div class='dataframe'>"
        html += pd.DataFrame(insights['statistical_summary']).to_html(classes='styled-table')
        html += "</div>"
    if 'strongest_correlations' in insights:
        html += "<h4>Strongest Correlations</h4><div class='dataframe'>"
        corr_df = pd.DataFrame(insights['strongest_correlations'].items(), columns=['Feature Pair', 'Correlation'])
        html += corr_df.to_html(index=False, classes='styled-table')
        html += "</div>"
    ...
    # --- Step 4, 5, & 6 ---
    html += "<div class='section'><h2>Step 4: Feature Engineering</h2>"
    ...
    html += "<div class='section'><h2>Step 5 & 6: Model Training & Evaluation</h2>"
    eval_results = results.get('training_evaluation', {})
    best_model = eval_results.get('best_model_name', 'N/A')
    best_score = eval_results.get('best_model_score', 0)
    html += f"<p><strong>Best Model Found:</strong> {best_model} (Score: {best_score:.4f})</p>"
    ...
    html += report_df.to_html(index=False, classes='styled-table')
    html += "</div></div>"
    
    return f"<!DOCTYPE html><html><head><title>AutoML Report</title>{style}</head><body><div class='container'>{html}</div></body></html>"

# --- 3. STREAMLIT APP LOGIC (Goes back to the simpler HTML download) ---
    ...
    if uploaded_file:
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(uploaded_file.name)[1]) as tmp:
                tmp.write(uploaded_file.getvalue()); tmp.seek(0)
                df_cols = pd.read_csv(tmp.name, nrows=0) if uploaded_file.name.endswith('.csv') else pd.read_excel(tmp.name, nrows=0)
            target = st.selectbox("Select Target Variable", options=[""] + df_cols.columns.tolist(), format_func=lambda x: "None (Unsupervised)" if x == "" else x)
...
if run_button:
    with st.spinner("Running the entire pipeline..."):
        with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(uploaded_file.name)[1]) as tmp:
...
if st.session_state.results:
    ...
    if "error" in results: st.error(f"An error occurred: {results['error']}")
    else:
        st.success("Pipeline finished successfully!")
        
        # UI DISPLAY LOGIC for all steps
        ...
        with st.expander("Step 3: Exploratory Data Analysis (EDA)", expanded=True):
            ...
            col1.metric("Rows", insights.get('shape', (0,0))[0])
            col2.metric("Columns", insights.get('shape', (0,0))[1])
            st.metric("Problem Type", insights.get('problem_type', 'N/A').capitalize())
            st.write("**Statistical Summary**")
            st.dataframe(pd.DataFrame(insights.get('statistical_summary', {})))
        ...
        # --- FINAL DOWNLOAD SECTION (HTML Report) ---
